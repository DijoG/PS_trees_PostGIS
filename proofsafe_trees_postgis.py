...
class ProofSafeGeoDB(ProofSafeTreeAPI):
    ...
    def __init__(self, 
                 base_url: str = "https://proofsafe-portalapi.tmo-gr.com",
        ...
        self.db_config = db_config if db_config else DB_CONFIG.copy()
    ...
    def log_import(self, 
        ...
        Args:
            ...
            table_name: Name of the table where data was stored
    ...
    def create_trees_table(self, table_name, drop_existing: bool = False) -> bool:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if drop_existing:
                    conn.execute(text("DROP TABLE IF EXISTS {table_name} CASCADE"))
                    conn.commit()
                ...
                CREATE INDEX idx_{table_name}_lat_lon ON {table_name} (latitude, longitude);
                CREATE INDEX idx_{table_name}_project ON {table_name} (project_id);
                CREATE INDEX idx_{table_name}_species ON {table_name} (lo3item_id);
                CREATE INDEX idx_{table_name}_health ON {table_name} (lo2item_id);
                CREATE INDEX idx_{table_name}_status ON {table_name} (status_id);
                CREATE INDEX idx_{table_name}_import_id ON {table_name} (import_id);
                CREATE INDEX idx_{table_name}_data_date ON {table_name} (data_date);
                """
                
                conn.execute(text(create_table_sql))
                conn.commit()
                
            print(f"✅ Created table '{table_name}' with PostGIS geometry column")
            return True
    ...
    def process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ===== FIX: Convert timestamp columns properly =====
        # DOB is a date column - convert from milliseconds
        if 'dob' in processed_df.columns:
            processed_df['dob'] = pd.to_datetime(processed_df['dob'], unit='ms', errors='coerce').dt.date
        
        # Created_At and Modified_At are timestamp columns
        ...
        # Updated_At is a bigint timestamp
    ...
    def store_trees(self, 
        ...
        if table_name not in inspector.get_table_names():
            print(f"⚠️ Table '{table_name}' doesn't exist. Creating it...")
            if not self.create_trees_table():
                return 0, False
        
        # Process DataFrame
        processed_df = self.process_dataframe(df)
        
        # Add metadata columns (THIS IS OPTION 2 - populating the timestamp columns)
        ...
        try:
            ...
            with self.engine.connect() as conn:
                conn.execute(text(f"""
                    CREATE TEMP TABLE trees_temp (LIKE {table_name} INCLUDING DEFAULTS) 
                    ON COMMIT DROP
                """))
            ...
            if debug:
                print(f"✅ Successfully stored {total_inserted} trees in database (Import ID: {import_id})")
    ...
    def fetch_and_store(self,
        ...
        # ===== TYPE DEFINITIONS BASED ON R STRUCTURE =====
        integer_columns = [
        ...
        try:
            ...
            for page_num, assets in enumerate(self.get_trees_generator(
                ...
                # ===== FIX: Lowercase all column names to match PostgreSQL =====
                ...
                processed_df = self.process_dataframe(df_page)  # Make sure process_dataframe also uses lowercase
                
                # Add metadata columns (these are already lowercase)
                processed_df['db_created_at'] = datetime.now()
                processed_df['db_updated_at'] = datetime.now()
                processed_df['data_date'] = datetime.now().date()
                processed_df['import_id'] = import_id
                
                # ===== SIMPLE FIX: Convert DataFrame types to match PostgreSQL =====
                ...
                try:
                    ...
                    for col in date_columns:
                        if col in processed_df.columns:
                            dtype_dict[col] = Date()  # This will work with datetime.date objects
                    ...
                    # Calculate geometry
                    from geoalchemy2 import Geometry
                    processed_df['geom'] = None  # Placeholder, will be calculated by database
                    
                    # Insert directly into the main table - pandas will use the correct types!
                    processed_df.to_sql(
                        table_name, 
                        self.engine, 
                        if_exists='append', 
                        index=False,
                        dtype=dtype_dict  # This is the key - tells pandas exactly what types to use
                    )
    ...
    def get_tree_count(self, table_name: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            if table_name is None:
                ...
                if latest:
                    table_name = latest[0]
                    print(f"🔍 Automatically detected latest table: '{table_name}'")
                else:
                    # Fallback to default 'trees' table
                    table_name = 'trees'
                    print(f"ℹ️ No trees_* tables found, checking default '{table_name}' table")
            ...
            with self.engine.connect() as conn:
                ...
                result = conn.execute(text(f"SELECT COUNT(*) FROM {table_name} WHERE geom IS NOT NULL"))
                with_geom = result.scalar()
                
                result = conn.execute(text(f"""
                    SELECT COUNT(DISTINCT project_id_name) FROM {table_name} 
                ...
                result = conn.execute(text(f"SELECT COUNT(DISTINCT import_id) FROM {table_name} WHERE import_id IS NOT NULL"))
                import_count = result.scalar() or 0
                
                result = conn.execute(text(f"""
                    SELECT MIN(data_date), MAX(data_date) FROM {table_name} 
                ...
                return {
                    ...
                    'table_auto_detected': table_name is None  # Flag indicating if table was auto-detected
                }
